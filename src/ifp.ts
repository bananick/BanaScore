...
/** Reverse map nom d'équipe (MAJ) → groupe, pour retrouver le groupe par le nom. */
const TEAM_TO_GROUP: Record<string, Group> = (() => {
  const m: Record<string, Group> = {};
  for (const g of GROUPS) for (const name of GROUP_TEAMS[g]) m[name.toUpperCase()] = g;
  return m;
})();

/** Groupe (A..F) d'une équipe d'après son nom (capitale), ou undefined. */
export function groupOfTeam(name: string): Group | undefined {
  return TEAM_TO_GROUP[name.trim().toUpperCase()];
}
